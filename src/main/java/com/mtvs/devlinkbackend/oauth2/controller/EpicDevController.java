...
@RestController
@RequestMapping("/api/user/epic/dev")
public class EpicDevController {
    private final EpicDevService epicDevService;
    private final JwtUtil jwtUtil;

    public EpicDevController(EpicDevService epicDevService, JwtUtil jwtUtil) {
    ...
    @PostMapping
    public ResponseEntity<UserPartnerSingleResponseDTO> registerDev(
            @RequestBody DevRequestDTO devRequestDTO,
            @RequestHeader("Authorization") String authorizationHeader) throws Exception {

        String accountId = jwtUtil.getSubjectFromAuthHeaderWithoutAuth(authorizationHeader);
        UserPartnerSingleResponseDTO userPartner = epicDevService.registUserPartner(devRequestDTO, accountId);
    ...
    @PatchMapping
    public ResponseEntity<UserPartnerSingleResponseDTO> update(
            @RequestBody DevRequestDTO devRequestDTO,
            @RequestHeader("Authorization") String authorizationHeader) throws Exception {

        String accountId = jwtUtil.getSubjectFromAuthHeaderWithoutAuth(authorizationHeader);
        UserPartnerSingleResponseDTO userPartner = epicDevService.updateUserPartner(devRequestDTO, accountId);
        return ResponseEntity.ok(userPartner);
    }
